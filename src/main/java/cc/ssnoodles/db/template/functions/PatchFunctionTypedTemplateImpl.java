 ...
 */
public class PatchFunctionTypedTemplateImpl implements FunctionTemplate {
    @Override
    public String getTemplate(Table table, String newClassName) {
        ...
        sb.append("MergePatch<").append(tableNameUpperCase).append(UPDATE).append("> patch) {").append(LINE)
                .append(SPACE).append(SPACE).append("var ").append(tableName).append(" = repository.load").append(tableNameUpperCase).append("(id).orElseThrow(ResourceNotFoundException::new);").append(LINE)
