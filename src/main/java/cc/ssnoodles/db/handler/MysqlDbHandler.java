...
public class MysqlDbHandler implements DbHandler {
    ...
    public void execute(List<Template> templates) throws SQLException {
        List<Table> tableList = getTables(ConnUtil.getConn(), DbType.MYSQL.getType(), USERNAME, null);
    ...
    public void execute(Template template, String tableName, String className) throws SQLException {
        List<Table> tableList = getTables(ConnUtil.getConn(), DbType.MYSQL.getType(), USERNAME, tableName);
